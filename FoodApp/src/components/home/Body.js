...
const Body = () => {
  ...
  const fetchData = async () => {
    const API =
      "https://www.swiggy.com/dapi/restaurants/list/v5?lat=25.4518146&lng=81.78734229999999&is-seo-homepage-enabled=true&page_type=DESKTOP_WEB_LISTING";

    const fetchedData = await fetch(API);
    const jsonFetchedData = await fetchedData.json();
    const restaurants =
      jsonFetchedData.data.cards[4].card.card.gridElements.infoWithStyle
        .restaurants;
  ...
  const handleSearch = () => {
    const searchedRes = listofRes.filter(
      (Res) =>
  ...
  return (
    <div className="searchAndRes ">
      <Offline className="offline-text text-center font-bold text-2xl">
        No internet
      </Offline>
      ...
      <div className="search-container p-6 text-lg">
        ...
        />
        <button
          className="hover:scale-105 duration-500 search mx-4  p-2 text-lg rounded-lg shadow-lg bg-lime-600 text-white"
      ...
      <div className="res-container flex flex-wrap">
        {arrayForRes &&
          arrayForRes.map((ResData) => (
            <Link
              className="res-link flex"
              key={ResData.info.id}
